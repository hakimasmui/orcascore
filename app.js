...
(async () => {
    const url = "https://www.goal.com/id/jadwal/2025-11-26";
...
async function crawlGaol(url, filename, tanggal_match, jsonArray) {
    ...
    const teams = ["SSC Napoli", "Slavia Praha", "Marseille", "Manchester City", "Barcelona", "Villarreal", "Juventus", "Ajax", "Galatasaray", "Al Hilal"];
    const league = ["International - Champions League", "International - AFC Champions League Elite"];
    let items = [];
    let tanggal;
    if (tanggal_match)
    ...
    if (allItems.length > 12) {
        allItems.splice(0, 3); // Hapus 3 item paling atas
    }
